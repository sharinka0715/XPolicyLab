...
mem0_legacy_run_id() {
    local dataset_name=$1 ckpt_name=$2 env_cfg_type=$3 expert_data_num=$4 action_type=$5 seed=$6
    echo "${dataset_name}-${ckpt_name}-${env_cfg_type}-${expert_data_num}-${action_type}-seed${seed}"
}
...
mem0_dataset_dir_candidates() {
    ...
    local tag
    tag="$(mem0_dataset_tag "${dataset_name}" "${ckpt_name}" "${env_cfg_type}" "${action_type}")"

    # Standard (README §4.2)
    echo "${policy_dir}/data/${tag}-lerobot"
    if [[ -n "${expert_data_num}" ]]; then
        echo "${policy_dir}/data/${dataset_name}-${ckpt_name}-${env_cfg_type}-${expert_data_num}-${action_type}-lerobot"
    fi

    # Legacy upstream layout
    if [[ -n "${expert_data_num}" ]]; then
        echo "${upstream_dir}/lerobot_datasets/${dataset_name}-${ckpt_name}-${env_cfg_type}-${expert_data_num}-${action_type}"
        echo "${upstream_dir}/lerobot_datasets/${dataset_name}-cotrain-${env_cfg_type}-${expert_data_num}-${action_type}"
    fi
...
mem0_resolve_dataset_dir() {
    ...
    done < <(mem0_dataset_dir_candidates "${policy_dir}" "${dataset_name}" "${ckpt_name}" \
        "${env_cfg_type}" "${action_type}" "${expert_data_num}")
    echo "${policy_dir}/data/$(mem0_dataset_tag "${dataset_name}" "${ckpt_name}" \
        "${env_cfg_type}" "${action_type}")-lerobot"
}

mem0_ckpt_dir_candidates() {
    local policy_dir=$1 dataset_name=$2 ckpt_name=$3 env_cfg_type=$4 action_type=$5 seed=$6
    ...
    legacy_id="$(mem0_legacy_run_id "${dataset_name}" "${ckpt_name}" "${env_cfg_type}" \
    ...
    if [[ -n "${expert_data_num}" ]]; then
        ...
        echo "${upstream_dir}/checkpoints/${dataset_name}-${ckpt_name}-${env_cfg_type}-${expert_data_num}-${action_type}-seed${seed}"
    fi
...
mem0_resolve_ckpt_dir() {
    ...
    local expert_data_num=${7:-}
    local candidate
    for candidate in $(mem0_ckpt_dir_candidates "${policy_dir}" "${dataset_name}" "${ckpt_name}" \
    ...
    echo "${policy_dir}/checkpoints/$(mem0_ckpt_run_id "${dataset_name}" "${ckpt_name}" \
...
mem0_planning_merged_candidates() {
    ...
    if [[ -n "${expert_data_num}" ]]; then
        echo "${policy_dir}/checkpoints/${dataset_name}-${ckpt_name}-${env_cfg_type}-${expert_data_num}-${action_type}-${seed}_planning_merged"
    fi
...
mem0_resolve_planning_merged_dir() {
    ...
    local expert_data_num=${7:-}
    local candidate
    for candidate in $(mem0_planning_merged_candidates "${policy_dir}" "${dataset_name}" "${ckpt_name}" \
        "${env_cfg_type}" "${action_type}" "${seed}" "${expert_data_num}"); do
        if [[ -d "${candidate}" ]]; then
            echo "${candidate}"
            return 0
...
mem0_default_dataset_out_dir() {
    ...
    if [[ "${MEM0_LEGACY_PATHS:-}" == "1" ]]; then
        local expert_data_num=${6:-}
        echo "${policy_dir}/Mem_0/lerobot_datasets/${dataset_name}-${ckpt_name}-${env_cfg_type}-${expert_data_num}-${action_type}"
    else
        echo "${policy_dir}/data/${tag}-lerobot"
    fi
}
