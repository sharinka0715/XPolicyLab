...
xpolicylab_dataset_dir_candidates() {
    ...
    if [[ -n "${expert_data_num}" ]]; then
        for root in "${roots[@]}"; do
            echo "${root}/${dataset_name}-${ckpt_name}-${env_cfg_type}-${expert_data_num}-${action_type}"
        done
    fi
    for root in "${roots[@]}"; do
        echo "${root}/cotrain_dataset"
        echo "${root}/${dataset_name}-${env_cfg_type}-${action_type}"
    done
}
...
xpolicylab_resolve_dataset_dir() {
    ...
    echo "${policy_dir}/data/$(xpolicylab_dataset_tag "${dataset_name}" "${ckpt_name}" "${env_cfg_type}" "${action_type}")"
}
...
xpolicylab_ckpt_dir_candidates() {
    ...
    echo "${policy_dir}/runs/${dataset_name}-${ckpt_name}-${env_cfg_type}-seed${seed}"
...
xpolicylab_resolve_checkpoint_pt() {
    ...
    ckpt_dir="$(xpolicylab_resolve_ckpt_dir "${policy_dir}" "${dataset_name}" "${ckpt_name}" \
